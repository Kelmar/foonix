/*************************************************************************/
/*
 * $Id: multboot.h 86 2010-10-17 19:14:34Z kfiresun $
 */
/*************************************************************************/
...
/*
 * NOTE:
 * We can't use "real" pointers because the multiboot loader
 * specification uses a 32 bit pointer for the structure.  The bootloader
 * also will not put us into the 64 bit mode, rather a 32 bit mode.
 *
 * If we wish to later develop a 64 bit OS, we will need to translate these
 * 32 bit pointers into 64 bit space at some point.
 */
/*************************************************************************/

struct mb_module_TYPE
{
...
typedef struct mb_module_TYPE mb_module_t;

/*************************************************************************/

struct mb_aout_syms_TYPE
{
    uint32_t tabsize;
    uint32_t strsize;
...
typedef struct mb_aout_syms_TYPE mb_aout_syms_t;

struct mb_elf_syms_TYPE
{
    uint32_t num;
    uint32_t size;
    uint32_t addr;
...
typedef struct mb_elf_syms_TYPE mb_elf_syms_t;

/*************************************************************************/

struct mb_memory_map_TYPE
{
...
typedef struct mb_memory_map_TYPE mb_memory_map_t;
...
typedef struct multiboot_TYPE multiboot_t;
