...
Kernel::ErrorCode uart::init()
{
    ...
    x86::outb(SERIAL_INT_CTL, 0);

    // Enable and reset the FIFO
    x86::outb(SERIAL_FIFO_CTL, 0x05);

    // Setup bit pattern, and enable writing ot the divisor latches
    x86::outb(SERIAL_LINE_CTL, SERIAL_CONTROL_FLAGS | SERIAL_DLA_BIT);

    // Set up for 19200 baud
    x86::outb(SERIAL_DLA_MSB, 0);
    x86::outb(SERIAL_DLA_LSB, 6);
...
int uart::read_char()
{
    ...
    if ((x86::inb(SERIAL_LINE_STAT) & 0x01) == 0)
        return -1;

    return x86::inb(SERIAL_TRX);
}
...
{
    while ((x86::inb(SERIAL_LINE_STAT) & 0x20) == 0)
        ;

    x86::outb(SERIAL_TRX, c);
}
